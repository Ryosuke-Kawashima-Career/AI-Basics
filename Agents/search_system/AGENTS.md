...
This is a Python 3.11 ADK/A2A dynamic deep search agent system. ADK agent entrypoints live in `agents/`, with one package per specialist plus `agents/coordinator/`. The coordinator is split across `agent.py`, `intake.py`, `candidates.py`, `evaluation.py`, `recommendation.py`, model files, and prompt helpers; keep workflow changes explicit there. The entry point is `src/main.py`.
...
- `make run-specialists`: start specialist A2A services on ports `8101`~`810X`.
